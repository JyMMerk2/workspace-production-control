...
export const AuthModal: React.FC<AuthModalProps> = ({ onLoginSuccess }) => {
  ...
  // Escuchar si el usuario regresa de una redirección OAuth de Google con Supabase
  useEffect(() => {
    supabase.auth.getSession().then(({ data: { session } }) => {
      if (session?.user) {
        const uEmail = session.user.email || '';
        const uName = session.user.user_metadata?.full_name || session.user.user_metadata?.name || uEmail.split('@')[0];
        const uPicture = session.user.user_metadata?.avatar_url || session.user.user_metadata?.picture;
        onLoginSuccess(uName, uEmail, uPicture);
      }
    });
  }, [onLoginSuccess]);
                ...
                {mode === 'LOGIN' && (
                  <div className="space-y-3 pb-3 border-b border-white/10">
                    ...
                    <button
                      ...
                      className="w-full py-2.5 px-4 bg-white/5 border border-white/15 rounded-full text-xs font-bold text-white hover:bg-white/10 transition-all flex items-center justify-center gap-2 cursor-pointer shadow-md"
                    >
                      ...
                      <span>Iniciar sesión con Google (Corporativo)</span>
                    </button>
